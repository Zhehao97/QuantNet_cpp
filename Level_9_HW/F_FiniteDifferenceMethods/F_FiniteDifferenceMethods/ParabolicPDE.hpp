...
namespace ParabolicIBVP{
    ...
    double (*BCL)(double t, std::string optType);                // The bottom boundary condition
    double (*BCR)(double t, std::string optType);                // The top boundary condition

    // Initial condition
    double (*IC)(double x, std::string optType);                 // The condition at time t = 0
}
